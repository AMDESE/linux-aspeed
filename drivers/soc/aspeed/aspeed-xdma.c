...
struct aspeed_xdma_kernel_client {
...
static int aspeed_xdma_release(struct inode *inode, struct file *file)
{
	...
	if (client == ctx->current_client) {
		/* Clear current_client first to prevent use-after-free */
		ctx->current_client = NULL;
	...
	if (reset) {
		/* Wait for any pending reset work to complete before freeing client */
		flush_work(&ctx->reset_work);
		aspeed_xdma_reset(ctx);
	}
...
/**
 * aspeed_xdma_get - Get a reference to an ASPEED XDMA engine
 * @dev: Device requesting the XDMA engine
 *
 * Looks up the XDMA device via device tree "aspeed,xdma" property
...
struct aspeed_xdma *aspeed_xdma_get(struct device *dev)
{
	...
	xdma_node = of_parse_phandle(dev->of_node, "aspeed,xdma", 0);
	if (!xdma_node)
		return ERR_PTR(-ENODEV);

	xdma_pdev = of_find_device_by_node(xdma_node);
	of_node_put(xdma_node);
	if (!xdma_pdev)
		return ERR_PTR(-EPROBE_DEFER);

	ctx = platform_get_drvdata(xdma_pdev);
	if (!ctx) {
...
EXPORT_SYMBOL_GPL(aspeed_xdma_get);

/**
 * aspeed_xdma_put - Release a reference to an ASPEED XDMA engine
 * @ctx: XDMA context
 */
...
EXPORT_SYMBOL_GPL(aspeed_xdma_put);

/**
 * aspeed_xdma_alloc_coherent - Allocate DMA-capable memory
 * @ctx: XDMA context
				  ...
				  dma_addr_t *dma_handle)
{
	...
	virt = gen_pool_dma_alloc(ctx->pool, size, dma_handle);
	if (virt)
		dev_dbg(ctx->dev, "Allocated %zu bytes at virt=%p dma=%pad\n",
			size, virt, dma_handle);

	return virt;
}
EXPORT_SYMBOL_GPL(aspeed_xdma_alloc_coherent);

/**
 * aspeed_xdma_free_coherent - Free DMA-capable memory
 * @ctx: XDMA context
				...
				void *vaddr, dma_addr_t dma_handle)
{
	...
	dev_dbg(ctx->dev, "Freeing %zu bytes at virt=%p dma=%pad\n",
		size, vaddr, &dma_handle);
	gen_pool_free(ctx->pool, (unsigned long)vaddr, size);
}
EXPORT_SYMBOL_GPL(aspeed_xdma_free_coherent);

/**
 * aspeed_xdma_transfer - Perform an asynchronous DMA transfer
 * @ctx: XDMA context
 * @bmc_addr: Physical address on BMC side
 * @host_addr: Physical address on host side
 * @len: Length of transfer
 * @upstream: Transfer direction (true = BMC->host, false = host->BMC)
...
int aspeed_xdma_transfer(struct aspeed_xdma *ctx, dma_addr_t bmc_addr,
			 ...
			 void *callback_data)
{
	struct aspeed_xdma_kernel_client *kclient;
	struct aspeed_xdma_op op;
	struct aspeed_xdma_cmd cmds[2];
	unsigned int num_cmds;
	int rc;
	...
	kclient = kzalloc(sizeof(*kclient), GFP_KERNEL);
	if (!kclient)
		return -ENOMEM;

	kclient->client.ctx = ctx;
	kclient->client.phys = bmc_addr;
	kclient->client.size = len;
	kclient->callback = callback;
	kclient->callback_data = callback_data;

	op.host_addr = host_addr;
	op.len = len;
	op.direction = upstream ? ASPEED_XDMA_DIRECTION_UPSTREAM :
				  ASPEED_XDMA_DIRECTION_DOWNSTREAM;

	num_cmds = ctx->chip->set_cmd(ctx, cmds, &op, bmc_addr);

	rc = aspeed_xdma_start(ctx, num_cmds, cmds, upstream,
			       &kclient->client);
	if (rc) {
		kfree(kclient);
		return rc;
...
EXPORT_SYMBOL_GPL(aspeed_xdma_transfer);

/**
 * aspeed_xdma_transfer_sync - Perform a synchronous DMA transfer
 * @ctx: XDMA context
 * @bmc_addr: Physical address on BMC side
 * @host_addr: Physical address on host side
 * @len: Length of transfer
			       ...
			       u64 host_addr, u32 len, bool upstream)
{
	...
	memset(&kclient, 0, sizeof(kclient));
	kclient.client.ctx = ctx;
	kclient.client.phys = bmc_addr;
	kclient.client.size = len;
	kclient.callback = NULL;
	...
	/* Wait for transfer to complete using the wait queue */
	rc = wait_event_interruptible(ctx->wait, !kclient.client.in_progress);
	if (rc)
		return -EINTR;

	return kclient.client.error ? -EIO : 0;
}
EXPORT_SYMBOL_GPL(aspeed_xdma_transfer_sync);
