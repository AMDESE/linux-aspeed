...
static ssize_t aspeed_mctp_netdev_tx(struct sk_buff *skb, struct net_device *ndev)
{
	...
	struct aspeed_mctp *priv = pcie_netdev->userdata;
	...
	pci_data_len_dw = FIELD_PREP(LEN_MASK_LO, FIELD_GET(PCI_VDM_HDR_LEN_MASK_LO, hdr_dw[0])) |
			FIELD_PREP(LEN_MASK_HI, FIELD_GET(PCI_VDM_HDR_LEN_MASK_HI, hdr_dw[0]));
	if (pci_data_len_dw == 0)
		pci_data_len_dw = SZ_1K;

	packet_data_sz_dw = tx_packet->size / sizeof(u32) -
			    sizeof(tx_packet->data.hdr) / sizeof(u32);
	if (packet_data_sz_dw != pci_data_len_dw) {
...
static int aspeed_mctp_register_netdev(struct aspeed_mctp *priv, int id)
{
	...
	return mctp_pcie_register_netdev(mctp_ifname, priv->dev, (u8 *)&mctp_addr,
					 priv, &aspeed_netdev_ops,
					 &priv->mctp_netdev);
}
