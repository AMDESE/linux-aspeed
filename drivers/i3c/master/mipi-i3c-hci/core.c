					      ...
					      struct device_node *np)
{
	...
	/*
	 * Workaround. Need to remove once hardware support is available for
	 * internal LDO powerup support. Check for the JESD403 compliance is
	 * added to avoid changing the frequency for the i3c buses like APML on
	 * which there are no i3c hub devices. This check can be removed when
	 * hardware support is available for internal LDO powerup.
	 */
	if (!i3c_device_power &&
	    hci->master.bus.context == I3C_BUS_CONTEXT_JESD403) {
...
#ifdef CONFIG_ARCH_ASPEED
	/*
	 * Driver should be able to send the CCC commands on the i3c buses like
	 * the APML/I3C bus on which there is no i3chub device. Following check
	 * for JESD403 is added so that the driver will not skip sending the CCC
	 * commands. Once the support of LDO internal powerup from the hardware
	 * is available, the check for JESD403 can be removed.
	 */
	if (!i3c_device_power &&
	    m->bus.context == I3C_BUS_CONTEXT_JESD403) {
