...
static void aspeed_i3c_of_populate_bus_timing(struct i3c_hci *hci, struct device_node *np)
{
	...
	/*
	 * Workaround for the i3c buses on which the i3c devices are powered
	 * from the external LDO. The i3c bus clock is set to 1MHz until the
	 * hardware support is available for internal LDO powerup. The check for
	 * JESD403 can be removed when hardware support is available for internal
	 * LDO powerup.
	 */
	if (!i3c_device_power &&
